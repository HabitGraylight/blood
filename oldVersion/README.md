...
> 归档说明：这是旧版 Node/SSE 原型，已移动到 `oldVersion/`。当前活跃版本在仓库根目录的 `newVersion/`，后续开发与部署请优先使用新版。
...
4. 游戏页负责城镇广场、夜晚顺序、提名投票、身份卡和必要的公开流程操作。
5. 社交页负责公开发言、私聊房和说书人私信。
6. 真人说书人模式下，房主可以看到魔典和说书人工具；LLM 说书人模式下，房主只能看到自己的身份，说书人建议、私聊总结和魔典日志会隐藏。
7. 和 AI 玩家对局时，房主可使用“AI 推进白天”：AI 会读取公开聊天后依次发言，然后服务端自动提名、投票、处决或进入夜晚。
8. 夜晚需要使用“结算夜晚”：首夜按规则不会发生小恶魔击杀；第二夜及以后小恶魔会自动选择目标，僧侣/士兵保护会阻止死亡。
...
- “AI 推进白天”会让存活 AI 逐个读取公开聊天、提名、票型和存活状态后发言，再自动完成一次提名投票并推进到夜晚。
...
- AI 白天自动流程会记录提名投票并推进到夜晚。
- 夜晚自动结算会跳过小恶魔首夜击杀，并在后续夜晚处理小恶魔击杀。
...
- 房间状态仍然是内存存储，服务重启会清空。
- 账号数据已持久化到 `data/`，但房间/对局状态暂未持久化。
- 角色能力目前只自动覆盖了部分关键流程；完整角色能力仍需继续扩展。
- 私聊目前是文本记录，不是语音/视频。

下一步建议优先做房间状态持久化、玩家换座/认领空座、完整角色能力钩子和更清晰的夜晚行动向导。
