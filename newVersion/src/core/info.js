 ...
 * 同时处理间谍/隐士的"误注册"机制。
 */
import { TEAM, resolveScript, rolesByTeam as scriptRolesByTeam, roleName as scriptRoleName } from "../scripts/registry.js";
import { effectiveRole } from "./setup.js";
import { hasStatus } from "./state.js";

function rolesByTeam(script, team) {
  return scriptRolesByTeam(script, team);
}
...
export function washerwomanInfo(players, self, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("washerwoman", { players, self, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function librarianInfo(players, self, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("librarian", { players, self, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function investigatorInfo(players, self, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("investigator", { players, self, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function chefInfo(players, self, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("chef", { players, self, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function empathInfo(players, self, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("empath", { players, self, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function fortuneTellerInfo(players, self, targetSeats, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("fortuneteller", { players, self, targets: targetSeats, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function undertakerInfo(players, executedSeat, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("undertaker", { players, self: players[executedSeat], executedSeat, corrupt, rng, script: resolveScript(scriptOrId) });
}

export function ravenkeeperInfo(players, targetSeat, corrupt, rng, scriptOrId) {
  return pickNightInfoCandidate("ravenkeeper", { players, self: null, targets: [targetSeat], corrupt, rng, script: resolveScript(scriptOrId) });
}
...
export function buildNightInfoOptions(roleId, context) {
  return buildNightInfoCandidates(roleId, context);
}
